...
class GetPhaseLoadUiUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<List<PhaseLoadItem>> {
        return activeDs.activeProjectId
            ...
            .flatMapLatest { projectId ->
                manualRepo.observeSession(projectId)
                    .flatMapLatest { session ->
                        // ✅ MANUAL: источник истины = draftState (overrideDao НЕ используется)
                        if (session?.manualModeActive == true) {
                            flowOf(buildPhaseItemsFromDraft(session.draftState))
                        } else {
                            // ✅ AUTO: текущая логика без регрессий (overrides работают)
                            combine(
    ...
    private fun buildPhaseItemsFromDraft(draft: ProjectEditState): List<PhaseLoadItem> {
        val deviceById = draft.devices.associateBy { it.deviceId }

        fun voltageFor(type: VoltageType): Double = when (type) {
            VoltageType.AC_3PHASE -> 400.0
            VoltageType.AC_1PHASE -> 230.0
            VoltageType.DC -> 230.0
        }

        fun nominalCurrent(powerW: Int?, voltageType: VoltageType, pf: Double?, demand: Double?): Double {
            val p = (powerW ?: 0).toDouble()
            val dr = (demand ?: 1.0).coerceIn(0.0, 1.0)
            return CurrentCalculator.calculateNominalCurrent(
                power = p,
                voltage = voltageFor(voltageType),
                powerFactor = pf,
                demandRatio = dr,
        ...
        val threePhasePairs = draft.groups
            .flatMap { g ->
                g.deviceIds.mapNotNull { id ->
                    val d = deviceById[id] ?: return@mapNotNull null
                    if (d.voltageType == VoltageType.AC_3PHASE) g to d else null
                }
            }
            ...
            threePhasePairs
                ...
                .map { items ->
                    ...
                    PhaseGroupItem(
                        ...
                        totalCurrent = deviceRows.sumOf { it.current }
                    )
                }
        ...
        // 2) A/B/C: группы фазы, внутри только НЕ 3φ устройства
        val phaseItems = listOf(Phase.A, Phase.B, Phase.C).map { phase ->
            val groupsOfPhase = draft.groups.filter { it.phase == phase }

            val groupRows: List<PhaseGroupItem> = groupsOfPhase.map { g ->
                val onePhaseDevices = g.deviceIds.mapNotNull { id ->
                    val d = deviceById[id] ?: return@mapNotNull null
                    if (d.voltageType != VoltageType.AC_3PHASE) d else null
                }
